...
public:
  _CCCL_HOST_DEVICE_API constexpr explicit __fmt_max_output_size(size_t __max_size) noexcept
  ...
  [[nodiscard]] _CCCL_HOST_DEVICE_API constexpr size_t __write_request(size_t __code_units) noexcept
  {
    const auto __result =
  ...
  [[nodiscard]] _CCCL_HOST_DEVICE_API constexpr size_t __code_units_written() const noexcept
  {
...
class __fmt_output_buffer
{
  ...
  [[nodiscard]] _CCCL_HOST_DEVICE_API constexpr size_t __available() const noexcept
  {
    return __capacity_ - __size_;
  }

  _CCCL_HOST_DEVICE_API constexpr void __prepare_write(size_t __code_units)
  {
...
public:
  ...
  _CCCL_HOST_DEVICE_API constexpr void __buffer_flushed() noexcept
  {
    __size_ = 0;
  }

  _CCCL_HOST_DEVICE_API constexpr void __buffer_moved(_CharT* __ptr, size_t __capacity) noexcept
  {
    __ptr_      = __ptr;
    __capacity_ = __capacity;
  }
  ...
  _CCCL_HOST_DEVICE_API constexpr void push_back(_CharT __c)
  {
  ...
  _CCCL_HOST_DEVICE_API constexpr void __copy(basic_string_view<_InCharT> __str)
  {
  ...
  _CCCL_HOST_DEVICE_API constexpr void __transform(_Iterator __first, _Iterator __last, _UnaryOperation __operation)
  {
  ...
  /// A \c fill_n wrapper.
  _CCCL_HOST_DEVICE_API constexpr void __fill(size_t __n, _CharT __value)
  {
    if (__max_output_size_)
    {
      __n = __max_output_size_->__write_request(__n);
  ...
  [[nodiscard]] _CCCL_HOST_DEVICE_API constexpr size_t __capacity() const noexcept
  {
    return __capacity_;
  }
  [[nodiscard]] _CCCL_HOST_DEVICE_API constexpr size_t __size() const noexcept
  {
    return __size_;
  }
};
...
class __fmt_allocating_buffer : public __fmt_output_buffer<_CharT>
{
  ...
  _CCCL_HOST_DEVICE_API constexpr void __grow_buffer(size_t __capacity)
  {
  ...
  _CCCL_HOST_DEVICE_API constexpr void __prepare_write(size_t __size_hint)
  {
    __grow_buffer(::cuda::std::max<size_t>(this->__capacity() + __size_hint, this->__capacity() * 1.6));
  }

  _CCCL_HOST_DEVICE_API static constexpr void __prepare_write(__fmt_output_buffer<_CharT>& __buffer, size_t __size_hint)
  {
    static_cast<__fmt_allocating_buffer<_CharT>&>(__buffer).__prepare_write(__size_hint);
  }

public:
  _CCCL_HOST_DEVICE_API constexpr explicit __fmt_allocating_buffer(
    __fmt_max_output_size* __max_output_size = nullptr) noexcept
  ...
  _CCCL_HOST_DEVICE_API _CCCL_CONSTEXPR_CXX20 ~__fmt_allocating_buffer()
  {
    if (__ptr_ != __small_buffer_)
    {
  ...
  [[nodiscard]] _CCCL_HOST_DEVICE_API constexpr basic_string_view<_CharT> __view() const noexcept
  {
...
class __fmt_direct_iterator_buffer : public __fmt_output_buffer<_CharT>
{
  ...
  _CCCL_HOST_DEVICE_API static constexpr void
...
public:
  _CCCL_HOST_DEVICE_API constexpr explicit __fmt_direct_iterator_buffer(
  ...
  [[nodiscard]] _CCCL_HOST_DEVICE_API constexpr _OutIt __out_it() &&
  {
...
class __fmt_container_inserter_buffer : public __fmt_output_buffer<_CharT>
{
  ...
  _CCCL_HOST_DEVICE_API constexpr void __prepare_write()
  {
...
public:
  _CCCL_HOST_DEVICE_API constexpr explicit __fmt_container_inserter_buffer(
    _OutIt __out_it, __fmt_max_output_size* __max_output_size = nullptr) noexcept
      : __fmt_output_buffer<_CharT>{__small_buffer_, __buffer_size, __prepare_write, __max_output_size}
      , __container_{__out_it.__get_container()}
  {}

  [[nodiscard]] _CCCL_HOST_DEVICE_API constexpr auto __out_it() &&
  {
    __container_->insert(__container_->end(), __small_buffer_, __small_buffer_ + this->__size());
...
public:
  _CCCL_HOST_DEVICE_API constexpr explicit __fmt_iterator_buffer(
